      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 border-border/40 bg-card/40 backdrop-blur-xl shadow-2xl relative overflow-hidden min-h-[500px]">
          ...
          <CardHeader>
            <CardTitle className="flex items-center gap-3 font-headline text-xl">
              <Globe className="text-primary h-6 w-6" /> {t.mapping.hotspotsTitle}
            </CardTitle>
          ...
          <CardContent className="h-[400px] flex items-center justify-center border-2 border-dashed border-border/20 m-6 rounded-2xl bg-secondary/5">
            <div className="text-center space-y-6">
              ...
              <p className="text-muted-foreground max-w-sm font-medium italic">
                {t.mapping.rendering} <br /> 
                <span className="text-[10px] font-black uppercase tracking-widest text-primary mt-2 block">{t.mapping.activeNodes}</span>
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          <Card className="border-border/40 bg-card/40 backdrop-blur-xl">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                <MapPin className="h-4 w-4 text-primary" /> {t.mapping.priorityRegions}
              </CardTitle>
            </CardHeader>
          ...
          <Card className="border-primary/20 bg-primary/5">
            <CardHeader>
              <CardTitle className="text-lg font-headline">{t.mapping.logistics}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground">{t.mapping.activeScouts}</span>
                <span className="font-black text-foreground">24</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground">{t.mapping.flightHours}</span>
                <span className="font-black text-foreground">1,240 hrs</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-muted-foreground">{t.mapping.reportsGenerated}</span>
    ...
    <div className="space-y-2 group">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground font-black uppercase tracking-tighter">{count} {t.mapping.identified} • <span className="text-accent">{growth}</span></p>
        </div>
        <div className="text-right">
          <p className="text-xs text-muted-foreground font-bold">{t.mapping.avgPim}</p>
